...
import { getBlogs, deleteBlogById } from 'apiSdk/blogs';
import { BlogInterface } from 'interfaces/blog';

type ColumnType = ColumnDef<BlogInterface, unknown>;
...
export function BlogListPage(props: BlogListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<BlogInterface>>(
    () => `/blogs?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: BlogInterface) => {
    if (hasAccess('blog', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/blogs/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/blogs/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
              ...
              {hasAccess('blog', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/blogs/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="blog" />
        </Flex>

        {hasAccess('blog', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/blogs/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
